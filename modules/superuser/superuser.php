...
class SuperUser extends Module
{
    ...
    protected function renderForm()
    {
        ...
        // Autocompletion, on génère le js ici, le html (champs de formulaire) est généré via HelperForm
        $html .= '<script type="text/Javascript">';
        $html .= "$(function() {\n";
        $html .= "$('.autocomp').autocomplete('/modules/superuser/ajax_userlist.php', {\n";
        $html .= "    dataType:'json', \n";
        $html .= "    parse:function(data){\n";
        $html .= "        var rows = new Array();\n";
        $html .= "        for( var i = 0; i<data.length; i++) {   \n";
        $html .= "            rows[i] = {data:data[i], value:data[i].company+' '+data[i].firstname+' '+data[i].lastname+' '+data[i].email, result:data[i].id_customer }; \n";
        $html .= "        }\n";
        $html .= "        return rows;   \n";
        $html .= "    },\n";
        $html .= "    formatItem: function(row, i, n) {\n";
        $html .= "        if (row.company==null) row.company=''; ";
        $html .= "        return row.id_customer + ' - ' + row.company +' '+row.firstname+' '+row.lastname+' '+row.email;\n";
        $html .= "    },\n";
        $html .= "    minChars:1, \n";
        $html .= "    autoFill:true, \n";
        $html .= "    max:20, \n";
        $html .= "    matchContains:false, \n";
        $html .= "    scroll:true, \n";
        $html .= "    selectFirst:true\n";
        $html .= "}).result(function(event, data, formatted) { \n";
        $html .= "    $('#clientSearchInput').val(formatted);";
        $html .= "    $('#SUPERUSER_CUSTOMERS').val(data.id_customer);";
        $html .= "});\n";
        $html .= "})";
        $html .= '</script>';
        // Fin modif
    ...
    protected function getConfigForm()
    {
        ...
        foreach ($customers_list as &$customer) {
            ...
            $customer['name'] = $customer['id_customer'].' - '.$customer['company'].' '.$customer['firstname'].' - '.$customer['lastname'].' - '.$customer['email'];
            ...
            'form' => array(
                ...
                'input' => array(
                    ...
                    /*array(
                        ...
                        'hint' => 'Choisissez le nom du client pour lequel vous souhaitez vous connecter en Front-office'
                    ),*/
                    // La liste déroulante est remplacée par un champs de recherche autocomplete et un champs hidden
                    array(
                        'type' => 'text',
                        'label' => $this->l('Customers'),
                        'name' => 'clientSearchInput',
                        'class' => 'autocomp',
                        'hint' => 'Tapez quelques lettres du nom pour trouver le client pour lequel vous souhaitez vous connecter en Front-office'
                    ),
                    array(
                        'type' => 'hidden',
                        'name' => 'SUPERUSER_CUSTOMERS'
    ...
    protected function getConfigFormValues()
    {
        ...
        $fields['SUPERUSER_CONNECT_BUTTON'] = '<div class="margin-form"><button type="submit" class="btn btn-primary button" name="submitSuperuserModule"><i class="icon-key"></i>'.$this->l('Connexion en mode client').'</button></div>';
