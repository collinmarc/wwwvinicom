...
{if version_compare($smarty.const._PS_VERSION_, '1.5', '<') || isset($customer.id_shop) && $customer.id_shop == $shop_ori}
    ...
    {if isset($customer.is_guest) && $customer.is_guest == '0'}
        ...
        <div style="overflow:hidden">
            ...
            <label for="use_last_cart" class="control-label">{l s='Restaurer le dernier panier' mod='superuser'}</label>
        ...
        <div class="row-margin-top margin-form">
            ...
            <button class="btn btn-default button" id="frontoffice_url" onClick="window.open('{$frontoffice_url|escape:'htmlall':'UTF-8'}'); return false;">{l s='Ouvrir la boutique pour' mod='superuser'} {$customer.firstname|escape:'htmlall':'UTF-8'} {$customer.lastname|escape:'htmlall':'UTF-8'} <i class="icon-external-link"></i></button>
