...
{capture name=path}<a href="{$link->getPageLink('my-account', true)|escape:'html':'UTF-8'}">{l s='Mon Compte'}</a><span class="navigation-pipe">{$navigationPipe}</span><span class="navigation_page">{l s='Ma commande' mod='shoppinglist'}</span>{/capture}

<h1 class="page-heading">{l s='Ma commande pré-établie'}</h1>
