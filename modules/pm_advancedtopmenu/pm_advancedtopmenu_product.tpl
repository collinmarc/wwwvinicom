<div id="adtm_product_{$column.id_column|intval}_{$column.productInfos.id_product|intval}" class="adtm_product{if version_compare($smarty.const._PS_VERSION_, '1.6.0.0', '>=')} adtm_product-16{/if}{if version_compare($smarty.const._PS_VERSION_, '1.6.0.0', '<') && version_compare($smarty.const._PS_VERSION_, '1.5.0.0', '>=')} adtm_product-15{/if}">
	...
	{* PrestaShop 1.5 Product Template *}
		<a href="{$product.link|escape:'html'}"{if $column.target} target="{$column.target|escape:'html':'UTF-8'}"{/if} title="{$product.name|escape:html:'UTF-8'}" class="product_image">
			<img src="{$link->getImageLink($product.link_rewrite, $product.id_image, $column.productSettings->p_image_type)|escape:'html'}" alt="{$product.name|escape:html:'UTF-8'}" />
		...
		<div class="adtm_product_buttons_container">
			...
			{if isset($column.productSettings->show_add_to_cart) && $column.productSettings->show_add_to_cart}
				{if ($product.id_product_attribute == 0 OR (isset($add_prod_display) AND ($add_prod_display == 1))) AND $product.available_for_order AND !isset($restricted_country_mode) AND $product.minimal_quantity == 1 AND $product.customizable != 2 AND !$PS_CATALOG_MODE}
					...
					<a class="exclusive ajax_add_to_cart_button" rel="ajax_id_product_{$product.id_product}" href="{$link->getPageLink('cart')|escape:'html'}?qty=1&amp;id_product={$product.id_product}&amp;token={$static_token}&amp;add" title="{l s='Add to cart' mod='pm_advancedtopmenu'}">{l s='Add to cart' mod='pm_advancedtopmenu'}</a>
					{else}
