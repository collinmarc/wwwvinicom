        ...
        switch ((int)Tools::getValue('entity')) {
            ...
            case $this->entities[$this->l('Products')]:
                ...
                $this->available_fields = array(
                    ...
                    'annee' => array('label' => $this->l('Millésime')),
                    'Conditionnement' => array('label' => $this->l('Conditionnement')),
                    'Contenant' => array('label' => $this->l('Contenant')),
                    'Couleur' => array('label' => $this->l('Couleur')),
    ...
    */
    ...
    public function productImport()
    {
        ...
        for ($current_line = 0; $line = fgetcsv($handle, MAX_LINE_SIZE, $this->separator); $current_line++) {
            ...
            $info = AdminImportController::getMaskedRow($line);
			//MCII
			// Suppression des * autour de la reférence et de l'ordre
			$info['reference'] = str_replace("*","",$info['reference']);
			$info['upc'] = str_replace("*","",$info['upc']);
			// Fusion des colonnes Features en une seule
			$info['features']="annee:" . $info["annee"] . ",conditionnement:" . $info["Conditionnement"] . ",couleur:" . $info["Couleur"] . ",Contenant:" . $info["Contenant"];
			// Suppression des colonnes fusionnées
			unset($info['annee']);
			unset($info['Conditionnement']);
			unset($info['Couleur']);
			unset($info['Contenant']);
